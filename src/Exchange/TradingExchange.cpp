...
namespace TradingEngine::Exchange {
    TradingEngine::Exchange::TradingExchange::TradingExchange(std::string yaml_file_path) : config_(YAML::LoadFile(yaml_file_path))
    {
    ...
    bool TradingEngine::Exchange::TradingExchange::tryGetOrderbook(Instrument::Security security, Orderbook::RetrievalOrderbook& orderbook)
    {
        auto it = orderbooks_.find(security.securityId_);
